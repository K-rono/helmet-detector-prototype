...
class SSDHelmetDetector(HelmetDetector):
	"""SSD300-VGG16 detector for helmet detection.

	Expects a checkpoint at models/ssd.pth with class indices:
	0: background, 1: DHelmet, 2: DNoHelmet, 3: DHelmetP1Helmet, 4: DNoHelmetP1NoHelmet
	"""

	def __init__(self, model_path: str = "models/ssd.pth", score_threshold: float = 0.23):
		if not os.path.exists(model_path):
			raise FileNotFoundError(f"SSD model not found at {model_path}")

		# 5 classes including background index 0
		self.model = torchvision.models.detection.ssd300_vgg16(num_classes=5)
	...
	def _preprocess(self, image: Image.Image):
		# Default torchvision transform: convert to tensor in [0,1]
	...
	def predict(self, image: Image.Image) -> DetectionResult:
		...
		for box, label, score in zip(outputs.get('boxes', []), outputs.get('labels', []), outputs.get('scores', [])):
			...
			# Updated counting logic for SSD (non-zero indexed)
